...
def issue_asset_api():
    try:
        response = {}
        data = request.json
        asset_amount = str(data['asset_amount'])
        asset_name = data['asset_name']
        asset_icon_url = data['asset_icon_url']
        create_token_response = subprocess.run(['spl-token', 'create-token'], check=True, stdout=subprocess.PIPE,
                                               text=True).stdout
        ...
        mint_token_response = subprocess.run(['spl-token', 'mint', asset_id, asset_amount], check=True,
                                             stdout=subprocess.PIPE, text=True).stdout
...
def send_asset_api():
    try:
        ...
        for token in tokens:
            if token["id"] == asset_id:
                sender_token_address = token["address"]
        send_token_response = subprocess.run(['spl-token', 'transfer', '--fund-recipient', sender_token_address,
                                              asset_amount, address],
